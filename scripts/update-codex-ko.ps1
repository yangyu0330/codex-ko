...
function New-CodexKoHelpMethod {
    $lines = @(
        ...
        "/plan - 구현 전에 읽을 파일, 수정 순서, 검증 방법을 먼저 합의합니다. 큰 작업은 여기서 범위를 고정한 뒤 시작하세요.",
        "/goal - 긴 작업 목표를 저장합니다. 여러 단계로 이어지는 작업에서 중간에 맥락을 잃지 않게 합니다.",
        ...
        "단계별 진행 - 1단계는 목표와 범위 정리, 2단계는 작은 변경 적용, 3단계는 /diff와 /review 확인으로 진행합니다.",
        "/new - 현재 터미널에서 새 대화를 시작합니다. 이전 대화와 맥락을 섞고 싶지 않을 때 씁니다.",
        "/resume - 이전에 저장된 대화를 다시 엽니다. 하던 작업을 이어갈 때 사용합니다.",
        "/quit - Codex를 종료합니다.",
        "",
        "처음 사용할 때 추천 흐름: /status로 현재 상태 확인, 작업 전 /init과 /plan으로 규칙과 계획 정리, 단계별 수정 후 /diff와 /review로 검토하기.",
